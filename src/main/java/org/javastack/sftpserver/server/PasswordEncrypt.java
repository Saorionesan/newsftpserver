...
import com.epoint.authenticator.asutils.sm.sm3.SM3Util;
...
public class PasswordEncrypt {
	...
	public static boolean isCrypted(final String input) {
		...
		if (input.startsWith("$1$") ||        // MD5
				...
				input.startsWith("$6$")||input.length()==64) {    // SHA2-512
			return true;
		}
		return false;
	}
	...
	public static boolean checkPassword(final String crypted, final String key) {
		...
		if (crypted.startsWith("$1$")) { // MD5
			crypted2 = Md5Crypt.md5Crypt(keyBytes.clone());
		} else if (crypted.startsWith("$apr1$")) { // APR1
			crypted2 = Md5Crypt.apr1Crypt(keyBytes.clone());
		} else if (crypted.startsWith("$5$")) { // SHA2-256
			crypted2 = Sha2Crypt.sha256Crypt(keyBytes.clone());
		} else if (crypted.startsWith("$6$")) { // SHA2-512
			crypted2 = Sha2Crypt.sha512Crypt(keyBytes.clone());
		} else {
            crypted2=SM3Util.encrypt(key);
		}
		Arrays.fill(keyBytes, (byte) 0);
		if (crypted2 == null) {
			return false;
